...
class Decoder(nn.Module):
    ...
    def __init__(self, L, data):
        ...
        self.decode_next = DecodeNext(len(data.alphabet), H)
    ...
    def forward(self, latent):
        ...
        y = torch.zeros(
            size=(batch_size, self.smile_len, len(self.alphabet)), 
            dtype=torch.float32, 
        ...
        inp = torch.full(
            fill_value=-32,
            size=(batch_size, 1, len(self.alphabet)), 
            dtype=torch.float32, 
            device=latent.device
        )
        inp[:, :, self.stoi['<BOS>']] = 32
        y[:, 0, :] = inp.squeeze()
